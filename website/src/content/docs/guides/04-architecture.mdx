...
│                    Your Application                      │
├─────────────────────────────────────────────────────────┤
│  TypeGit (convenience)  │  createGit() (advanced)       │
├─────────────────────────────────────────────────────────┤
│         Git              │       WorktreeRepo / BareRepo │
│   (repo-agnostic)        │       (repo-specific)         │
├─────────────────────────────────────────────────────────┤
│                      CliRunner                           │
│            (command construction, progress)              │
├─────────────────────────────────────────────────────────┤
│     ExecAdapter          │         FsAdapter             │
│   (process spawning)     │    (file operations)          │
├─────────────────────────────────────────────────────────┤
│  Node.js  │    Deno     │     Bun                        │
└─────────────────────────────────────────────────────────┘
```
...
WorktreeRepoImpl.status()
    │ Builds args: ['status', '--porcelain=v2']
    ▼
CliRunner.run(context, args)
    │ Adds context: ['git', '-C', '/path/to/repo', ...]
    │ Adds env: { GIT_LFS_FORCE_PROGRESS: '1', ... }
    ▼
...
CliRunner
    ...
    ▼
WorktreeRepoImpl
    │ Parses stdout with parsePorcelainV2()
    ▼
StatusPorcelain (typed result)
```
